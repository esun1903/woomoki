          ...
          <validation-observer v-slot="{ invalid }">
            <v-form @submit.prevent="submit">
              <validation-provider v-slot="{ errors }" name="phone" rules="required|integer|length:11">
                <v-text-field v-model="phone" :error-messages="errors" label="Phone"></v-text-field>
              </validation-provider>

              <div class="text-center">
                <v-btn class="find-id-btn" type="submit" rounded color="white" :disabled="invalid">
                  인증 번호 보내기
                </v-btn>
...
<script>
  import { required,integer,length } from 'vee-validate/dist/rules'
